...
const EventManager = {
    possibleEvents: [ // Глобальные события, происходящие на базе или в мире
        ...
        },
        {
            ...
            text: "К базе подошел торговец. Предлагает 3 'Стимулятора' за 15 'Металлолома' (с вашего личного инвентаря).", // Убедитесь, что stimpack_fallout есть в items.js
            choices: [
                {
                    ...
                    action: function() {
                        if (typeof InventoryManager !== 'undefined' && InventoryManager.countItemInInventory(gameState.inventory, "scrap_metal") >= 15) {
                            ...
                            InventoryManager.addItemToInventory(gameState.inventory, "stimpack_fallout", 3); // Предполагаем, что stimpack_fallout - это ID предмета
                ...
                {
                    text: "Отказаться",
                    action: function() { if (typeof game !== 'undefined' && game.log) game.log("Торговец ушел, ворча себе под нос.", "event-neutral"); }
                }
            ]
        },
        {
                    ...
                    action: function() {
                        if (typeof InventoryManager !== 'undefined' && InventoryManager.countItemInInventory(gameState.baseInventory, "wood") >=5 && InventoryManager.countItemInInventory(gameState.baseInventory, "scrap_metal") >=3){
                            ...
                            if (typeof game !== 'undefined' && game.log) game.log("Оборона усилена. Зомби не решились атаковать.", "event-positive");
                             ...
                             if(!gameState.gameOver && typeof game !== 'undefined' && game.takeDamage) game.takeDamage(10 + (gameState.day / 2), "атака зомби на базу"); // Урон фиксированный + небольшой бонус от дня
                             
                             let diedInHordeAttack = 0;
                             for (let i = 0; i < gameState.survivors; i++) {
                                 if (Math.random() < 0.15) { // 15% шанс для каждого выжившего умереть
                                     diedInHordeAttack++;
                                 }
                             }
                ...
                },
                {
                    ...
                    action: function() {
                         if (Math.random() < 0.6) {
                            ...
                            if(!gameState.gameOver && typeof game !== 'undefined' && game.takeDamage) game.takeDamage(15 + gameState.day, "внезапная атака на базу"); // Урон выше и сильнее зависит от дня

                            let diedInSuddenAttack = 0;
                            for (let i = 0; i < gameState.survivors; i++) {
                                if (Math.random() < 0.25) { // 25% шанс для каждого выжившего умереть
                                    diedInSuddenAttack++;
                                }
                            }
    ...
    triggerRandomEvent: function() {
        ...
        if (availableEvents.length > 0) {
            gameState.currentEvent = { ...availableEvents[Math.floor(Math.random() * availableEvents.length)] }; // Копируем объект события
            if (typeof game !== 'undefined' && game.log) game.log(`СОБЫТИЕ: ${gameState.currentEvent.text}`, "event-discovery");
            this.displayEventChoices();
        } else {
            // Если нет глобальных событий, и активна вкладка разведки, убедимся, что контейнер событий скрыт (если там не локальное событие)
            const exploreTabActive = document.getElementById('explore-tab')?.style.display === 'block';
            if (exploreTabActive && domElements.eventActionsContainer && !gameState.locationEvent) {
    ...
    displayEventChoices: function() {
        ...
        if (!gameState.currentEvent) {
            domElements.eventActionsContainer.style.display = 'none';
            // UIManager.updateExploreTabDisplay(); // Это вызывается из finalizeEventUI -> updateAllUI -> updateForTab
            return;
        }
        ...
        // Блокируем действия на время глобального события
        ...
        // Другие кнопки действий, которые должны быть заблокированы, например, "пропустить день на базе"
        const passDayBaseButton = document.getElementById('pass-day-base-button'); // Если такая кнопка будет
        ...
        gameState.currentEvent.choices.forEach(choice => {
            ...
            if (typeof choice.action === 'function') {
                btn.onclick = () => {
                    if (gameState.currentEvent && choiceIsAvailable) { // Дополнительная проверка на доступность выбора
                        try {
                            choice.action();
                        } catch (e) {
                 ...
                 btn.disabled = true; // Если нет действия, кнопка бесполезна
            }
    ...
    displayLocationEventChoices: function() {
        ...
        if (!gameState.locationEvent) {
            domElements.eventActionsContainer.style.display = 'none';
            // UIManager.updateExploreTabDisplay(); // Это вызывается из finalizeEventUI
            return;
        }
        ...
        gameState.locationEvent.choices.forEach(choice => {
            ...
            const handleChoice = () => {
                if (gameState.locationEvent && choiceIsAvailable) {
                    const currentLocEvent = { ...gameState.locationEvent }; // Копируем, т.к. gameState.locationEvent станет null
                    gameState.locationEvent = null; // Сбрасываем событие до выполнения действия
                    ...
                    try {
                        if (typeof choice.action === 'function') {
                            ...
                            // Дополнительные возможные исходы: урон, изменение статов и т.д.
                        }
                    } catch (e) {
                    ...
                    // Если событие локации подразумевало трату времени (например, бой или долгое взаимодействие),
                    // то здесь или в самом action/outcome должен быть вызов LocationManager.nextDayForLocationAction()
                    // Это важный момент для корректного течения времени.
                    // Пока что, для простоты, предполагаем, что выбор в событии локации времени не тратит,
                    // и день продвинулся до события.
                    if (typeof UIManager !== 'undefined') UIManager.finalizeEventUI();
                }
            };
